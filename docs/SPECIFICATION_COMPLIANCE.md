...
## 2. 実装状況（2025年12月時点の記録）

> **注記**: 本章が説明している `src/parse_xml.py` を含む旧JSONパイプラインの
> スクリプトは 2026-08-28 に削除しました（Issue #13）。以降の記述は当時の
> 準拠性分析の記録であり、現行の実装ではありません。現在の本文抽出は
> PMDA公式XSLTによるレンダリング（`src/render_xsl.py` / `src/xml_to_db.py`）に
> 置き換わっています。構成は `CLAUDE.md` を参照してください。

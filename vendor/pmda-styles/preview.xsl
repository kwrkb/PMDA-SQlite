...
<xsl:variable name="file">label-<xsl:value-of select="$lang"/>.xml</xsl:variable>
<xsl:variable name="label" select="document($file)/label"/>

<!-- その他設定ファイル -->
<xsl:variable name="standardname" select="document('StandardName.xml')/StandardNames"/>
<xsl:variable name="regclass" select="document('RegulatoryClassification.xml')/RegulatoryClassifications"/>
...
<xsl:include href="preview-include.xsl" />

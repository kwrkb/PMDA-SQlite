...
def test_parser():
    ...
    for product_name, expected_form, expected_strength, expected_unit in test_cases:
        result = parse_product_name(product_name)

        # テスト結果の検証
        form_ok = result['dosage_form'] == expected_form
        strength_ok = result['strength'] == expected_strength
        unit_ok = result['strength_unit'] == expected_unit

        if form_ok and strength_ok and unit_ok:
            success += 1
            status = "✓"
        else:
            status = "✗"
        ...
        print(f"   剤形: {result['dosage_form']} (期待: {expected_form})")
        print(f"   含有量: {result['strength']} (期待: {expected_strength})")
        print(f"   単位: {result['strength_unit']} (期待: {expected_unit})")
        print()

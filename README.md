...
| `heading` | 項番を除いた見出し文言 |
| `level` | 階層（`1`〜`4`。`99`は「階層なし」の番兵値）。**空文字にもなりうる**（公式XSLが `data-level` 属性を付けないセクション。本番DBで98,470件＝12.2%）ので、`level IN ('1','2','3','4','99')` のような絞り込みは取りこぼす |
| `body_md` | Markdown化した本文。項番付きの箇条書きは `- 2.1 本剤の…` の形（項番はPMDA公式XSLが本文に書いた値で、こちら側では番号を振らない）。Markdownに対応する記法が無い要素はHTMLタグのまま埋め込む: `rowspan`/`colspan` を含む表、上付き・下付き（`10<sup>5</sup>`）。図は `![図](<ファイル名>)`、外部リンクは `[text](url)` |
